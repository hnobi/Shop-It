		...
		return (
			<div className={`shopping-cart ${newClass}`}>
				<div className="shopping-cart__body">
					<p className="shopping-cart__cancel-btn" onClick={() => this.props.cancelBtn()}>
						✕
					</p>
									...
									return (
										<tr key={i}>
											<td className="shopping-cart__item">
												<img src={`${img}/${item.image}`} className="shopping-cart__img" />
												<div className="shopping-cart__details">
													<h4 className="shopping-cart__item__title">{item.name}</h4>
				...
				<footer className="footer">
					<button className="footer-back">Back to Shop</button>
					<button className="footer-checkout">Checkout</button>
				</footer>
			</div>
		);
	}
